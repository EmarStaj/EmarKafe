...
class _AdminScreenState extends State<AdminScreen> {
              ...
              children: [
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText: 'Kısa açıklama (kartlarda görünür)',
                  ),
                ),
                const SizedBox(height: 10),
                TextField(
                  ...
                  decoration: const InputDecoration(
                    labelText:
                        'Detay metni (dokununca açılır — boş bırakılırsa kısa açıklamadan üretilir)',
                  ),
                ),
                const SizedBox(height: 10),
                TextField(
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (title.isEmpty || subtitle.isEmpty) {
                  setDialogState(
                    () => error = 'Başlık ve kısa açıklama zorunlu.',
                  );
                  return;
                }
                final colors =
                    _campaignColorPool[_colorPoolIndex %
                        _campaignColorPool.length];
                _colorPoolIndex++;
                app.addCampaign(
                  Campaign(
                    title: title,
                    ...
                    details: detailsCtrl.text.trim().isEmpty
                        ? '$subtitle Detaylar için şubene sorabilirsin.'
                        : detailsCtrl.text.trim(),
                    badge: badgeCtrl.text.trim().isEmpty
                        ? 'YENİ'
                        : badgeCtrl.text.trim().toUpperCase(),
                    icon: '🎁',
                    colors: colors,
                  ),
                );
            ...
            child: Column(
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  onChanged: (v) =>
                      setDialogState(() => selectedRole = v ?? 'barista'),
                ),
                const SizedBox(height: 10),
                DropdownButtonFormField<String?>(
              ...
              onPressed: isLoading
                  ...
                  : () async {
                      ...
                      if (selectedBranchId == null) {
                        setDialogState(
                          () => errorMsg = 'Lütfen bir şube seçin.',
                        );
                        return;
                      }
                      setDialogState(() {
  ...
  Future<void> _deleteStaff(AppState app, StaffMember s) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Text(
          '${s.fullName} adlı personel silinecek. Bu işlem geri alınamaz.',
        ),
        actions: [
          TextButton(
    ...
    if (confirm == true && mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              ok ? '${s.fullName} silindi.' : 'Hata: ${app.staff.error}',
            ),
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    final hotCount = app.menu.products
        .where((p) => p.category == ProductCategory.hotCoffee)
        .length;
    final icedCount = app.menu.products
        .where((p) => p.category == ProductCategory.icedCoffee)
        .length;
    final dessertCount = app.menu.products
        .where((p) => p.category == ProductCategory.dessert)
        .length;

    final ranked = List.of(app.branches)
      ..sort((a, b) => _orderCountFor(b).compareTo(_orderCountFor(a)));
    final maxCount = ranked.isEmpty ? 1 : _orderCountFor(ranked.first);
    final totalOrders = app.branches.fold(
      0,
      (sum, b) => sum + _orderCountFor(b),
    );
          ...
          children: [
            GridView.count(
              ...
              children: [
                ...
                _StatTile(
                  n: '${hotCount + icedCount}+$dessertCount',
                  l: 'Kahve + Tatlı',
                ),
              ],
            ),
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  '${app.orderHistory.length} sipariş',
                  style: TextStyle(
                    fontSize: 11.5,
            ...
            Builder(
              builder: (context) {
                final filtered = app.orderHistory
                    .where(
                      (o) =>
                          _orderBranchFilter == 'Tümü' ||
                          o.branch == _orderBranchFilter,
                    )
                    .toList();
                if (filtered.isEmpty) {
                  return Padding(
                ...
                return Column(
                  children: filtered.map((o) {
                    ...
                    final statusLabel = o.pickedUp
                        ? 'Teslim Alındı'
                        : (ready
                              ? 'Hazır'
                              : (o.computedStatus == OrderStatus.preparing
                                    ? 'Hazırlanıyor'
                                    : 'Alındı'));
                    final statusColor = o.pickedUp
                        ? EmarColors.espresso.withValues(alpha: 0.4)
                        : (ready ? EmarColors.moss : EmarColors.paprika);
                    return Container(
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  '${o.branch} · ${o.total.toStringAsFixed(0)}₺',
                                  style: TextStyle(
                                    fontSize: 10.5,
                                    color: EmarColors.espresso.withValues(
                                      alpha: 0.55,
              ...
              Padding(
                ...
                child: Text(
                  app.staff.error != null
                      ? 'Personel yüklenirken hata: ${app.staff.error}'
                      : 'Henüz personel yok. Sağ alttaki butona tıklayarak ekleyin.',
                  style: TextStyle(
                    fontSize: 12,
                    color: EmarColors.espresso.withValues(alpha: 0.5),
                  ),
                ...
                (s) => Container(
                  ...
                  child: Row(
                    children: [
                      CircleAvatar(
                        radius: 15,
                        backgroundColor: s.role == 'branch_manager'
                            ? EmarColors.gold
                            : EmarColors.moss,
                        child: Text(
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              '${s.email ?? '-'} · ${s.roleLabel}${s.branchName != null ? ' · ${s.branchName}' : ''}',
                              style: TextStyle(
                                fontSize: 10.5,
                                color: EmarColors.espresso.withValues(
                                  alpha: 0.55,
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: 'Yeni şube adı (ör. Samsun – İlkadım)',
                    ),
                  ),
                ),
              ...
              children: [
                ...
                ActionChip(
                  ...
                  onPressed: () => ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Menü düzenleme — sıradaki adım'),
                    ),
                  ),
                ),
              ],
